...
test("String.resolve basic (one dimension) test", 1, function () {
    // given
    var values = { first: "test", second: "case" };
    var stringToResolve = "{{first}} {{second}}";
    // when
...
test("String.resolve advanced (n dimension) test", 1, function () {
    ...
    var values = {
        ...
        third: {
            more: "more",
            adv: { test: "advanced test" },
            "case": "case"
        }
    };
    var stringToResolve = "{{first.sub}} {{second}} {{third.more}} {{third.adv.test}} {{third.case}}";
    // when
    var result = stringToResolve.resolve(values);
    // then
